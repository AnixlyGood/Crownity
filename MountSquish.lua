...
local Window = AnixlyUI:CreateWindow({
    Title = "Anixly Hub",
    Subtitle = "Version 1.0.0 | " .. executorName .. " | Mount Squish", -- FIX: tambahin ..
    Theme = "ANIXLY",
...
--// AUTO SUMMIT MOUNT SQUISH (CP1 - CP21)
local AutoSummitSection = MainTab:AddSection("🏔️ Auto Summit Mount Squish")
...
local function TeleportToCP(cpNumber)
    ...
    if not checkpointParent then
        checkpointParent = workspace:FindFirstChild("MountSquish") or workspace:FindFirstChild("Mount")
    end
...
AutoSummitSection:AddToggle({
    ...
    Callback = function(value)
        if value then
            ...
            loopCount = 0
            StartAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Squish: Enabled", "success", 3)
        else
            StopAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Squish: Disabled", "info", 2)
        end
    end
})
