...
--// AUTO SUMMIT MOUNT YOUR DREAM (CP1 - CP40)
local AutoSummitSection = MainTab:AddSection("🏔️ Auto Summit Mount Your Dream")

local autoSummitEnabled = false
local currentCp = 1
local cpDelay = 1
local loopCount = 0
local selectedTargetCP = "CP40"
...
local function TeleportToCP(cpNumber)
    ...
    if not checkpointParent then
        checkpointParent = workspace:FindFirstChild("MountYourDream") or workspace:FindFirstChild("Mount")
    end
...
AutoSummitSection:AddToggle({
    Text = "🏔️ Auto Summit Mount Your Dream",
    Default = false,
    Callback = function(value)
        if value then
            ...
            loopCount = 0
            StartAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Your Dream: Enabled", "success", 3)
        else
            StopAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Your Dream: Disabled", "info", 2)
        end
    end
})
