...
if CONFIG.BLOCK_REMOTES then
    ...
    local allowedRemotes = {
        ...
        "TeleportTo", "CP", "MountSalora", "MountMahoni", "MountSawit"
    }
...
local Window = AnixlyUI:CreateWindow({
    Title = "Anixly Hub",
    Subtitle = "Version 1.0.0 | " .. executorName .. " | Mount Sawit",
    Theme = "ANIXLY",
...
--// AUTO SUMMIT MOUNT SAWIT (Checkpoint1 - Checkpoint40)
local AutoSummitSection = MainTab:AddSection("🌴 Auto Summit (Mount Sawit) ")

local autoSummitEnabled = false
local currentCp = 1
local cpDelay = 1
local loopCount = 0
local selectedTargetCP = "Checkpoint40"
...
local function TeleportToCP(cpNumber)
    ...
    if not checkpointParent then
        checkpointParent = workspace:FindFirstChild("MountSawit") or workspace:FindFirstChild("Mount")
    end
    ...
    -- Coba dengan format "Checkpoint1", "Checkpoint2", dst
    local cpName = "Checkpoint" .. cpNumber
    local cp = checkpointParent:FindFirstChild(cpName)
    
    -- Jika tidak ditemukan, coba format "CP1", "CP2", dst
    if not cp then
        cpName = "CP" .. cpNumber
    ...
    if not cp then
        for _, child in ipairs(checkpointParent:GetChildren()) do
            if child.Name:lower() == "checkpoint" .. cpNumber or child.Name:lower() == "cp" .. cpNumber then
                cp = child
                break
            end
        end
...
local function StartAutoSummit()
    ...
    local targetNum = 40
    if selectedTargetCP and selectedTargetCP ~= "All Checkpoints" then
        targetNum = tonumber(selectedTargetCP:match("%d+")) or 40
    end
    
    Notify("AUTO SUMMIT", "Loop #" .. loopCount .. " - Starting from Checkpoint1 to " .. selectedTargetCP, "info", 2)
...
-- Buat list Checkpoint1-Checkpoint40
local cpOptions = {}
for i = 1, 40 do
...
AutoSummitSection:AddToggle({
    ...
    Callback = function(value)
        if value then
            ...
            loopCount = 0
            StartAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Sawit: Enabled", "success", 3)
        else
            StopAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Sawit: Disabled", "info", 2)
        end
    end
})

AutoSummitSection:AddSlider({
    Text = "⏱️ Delay between Checkpoints (seconds)",
    Min = 1,
    Max = 5,
    Default = 1,
    Callback = function(value)
...
local function FullBypass()
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {
        ...
        Text = "All bypass methods applied successfully!",
        Duration = 5
    })
end

-- Auto run all bypasses
task.spawn(function()
    FullBypass()
end)
