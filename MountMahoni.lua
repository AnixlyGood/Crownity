...
--// AUTO SUMMIT MOUNT MAHONI (CP1 - CP30)
local AutoSummitSection = MainTab:AddSection("🏔️ Auto Summit Mount Mahoni")

local autoSummitEnabled = false
local currentCp = 1
local cpDelay = 1
local loopCount = 0
local selectedTargetCP = "CP30"
...
local function TeleportToCP(cpNumber)
    ...
    if not checkpointParent then
        checkpointParent = workspace:FindFirstChild("MountMahoni") or workspace:FindFirstChild("Mount")
    end
...
local function StartAutoSummit()
    ...
    if selectedTargetCP and selectedTargetCP ~= "All CP" then
        targetNum = tonumber(selectedTargetCP:match("%d+")) or 30
    end
...
AutoSummitSection:AddToggle({
    Text = "🏔️ Auto Summit Mount Mahoni",
    Default = false,
    Callback = function(value)
        if value then
            ...
            loopCount = 0
            StartAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Mahoni: Enabled", "success", 3)
        else
            StopAutoSummit()
            Notify("AUTO SUMMIT", "Auto Summit Mount Mahoni: Disabled", "info", 2)
        end
    end
})
