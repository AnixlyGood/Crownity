...
Library._spawns = {}
Library._saveThread = nil -- FIX: track autosave thread for cancellation
...
-- FIX: Cleanup now also cancels saveThread and clears CallbackRegistry + flags
function Library:Cleanup()
    ...
    -- FIX: Cancel pending autosave thread
    ...
    -- FIX: Clear CallbackRegistry so zombie callbacks don't fire after re-init
    table.clear(CallbackRegistry)
    -- FIX: Clear flags so stale GUI references are released
...
local DefaultConfig = {}
local isDirty = false
-- FIX: CallbackRegistry is module-level but cleared on each Cleanup/CreateWindow
...
-- FIX: MarkDirty now cancels the previous thread before scheduling a new one,
-- preventing orphaned task.delay threads accumulating over time.
...
function Library:CreateWindow(config)
    ...
    -- FIX: Clear stale state before creating a new window
    table.clear(CallbackRegistry)
    table.clear(self.flags)
    table.clear(self.pages)
    table.clear(self._navButtons)
    ...
    local function createMinimizedIcon()
        ...
        -- FIX: Store icon drag connections locally and disconnect on icon destroy
        local iconConns = {}
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if dragging then
                    ...
                    if not dragMoved then
                        ...
                        -- Disconnect all icon connections before destroying
        ...
        -- FIX: Disconnect icon connections when icon is destroyed externally (e.g. GUI cleanup)
    ...
    -- FIX: Register header drag connection properly
    ...
    -- FIX: Register resize connection properly
    ...
    -- FIX: Full cleanup on GUI destroy, including saveThread and CallbackRegistry
...
function Library:CreatePage(name, title, imageId, order)
    ...
    -- FIX: Register nav button connections so they disconnect on Cleanup
...
function Library:CreateCategory(parent, title)
    ...
    local isOpen = false
    -- FIX: Register category toggle connection so it disconnects on Cleanup
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    ...
    -- FIX: Register toggle button connection
...
function Library:_initDropdownSystem()
    ...
    -- FIX: Register close overlay connection
...
function Library:CreateDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue)
    ...
    -- FIX: Only update CanvasSize from AbsoluteContentSize listener (single source of truth)
    ...
    -- FIX: Cache option frames for faster search filtering (avoid GetChildren() each keystroke)
    ...
    -- FIX: Debounced search â€” cancel previous search task before scheduling new one
    ...
    self:AddConnection("searchBox_" .. dropdownLayoutOrder, searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        searchThread = task.delay(0.08, function()
            searchThread = nil
            -- FIX: Use cache instead of GetChildren() each time
            for _, entry in ipairs(optionFrameCache) do
    ...
    -- FIX: Register dropdown open button connection
...
function Library:CreateMultiDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValues)
    ...
    -- FIX: Single source of truth for CanvasSize
    ...
    -- FIX: Cache option frames for faster search filtering
    local optionFrameCache = {}
    ...
    function DropdownFunc:AddOption(option)
        ...
        -- FIX: Register multi-dropdown option button connection
        ...
        -- FIX: Cache frame for fast search
        table.insert(optionFrameCache, {frame = optionFrame, lowerLabel = string.lower(label)})
    end
    ...
    -- FIX: Debounced search using cached option frames
    local searchThread = nil
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    -- FIX: Register FocusLost connection
...
function Library:CreateButton(parent, label, callback)
    ...
    -- FIX: Track active tween so it can be cancelled if frame is destroyed mid-animation
    ...
    self:AddConnection("btn_" .. label .. tostring(button), button.MouseButton1Click:Connect(function()
        ...
        -- Cancel any ongoing tween before starting new one
        if activeTween then
            activeTween:Cancel()
            activeTween = nil
        end
...
function Library:_createConfigTab(WindowObject)
    ...
    mgmtSection:AddParagraph({
        ...
        Content = "Setelah melakukan Reset to Default, beberapa settingan seperti Toggle dan nilai Input akan langsung ter-update di UI.\n\n"
               .. "Namun untuk settingan yang mempengaruhi karakter, kecepatan, atau fitur aktif lainnya â€” kamu perlu Rejoin / Respawn agar perubahan berlaku sepenuhnya.\n\n"
